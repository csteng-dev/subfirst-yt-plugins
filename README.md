# 🛡️ SubFirst (YouTube 訂閱優先) — 演算法過濾與專注盾
...
3. **載入開發者擴充功能**：
   ...
   - 點擊「選擇資料夾」。
4. **安裝成功**：
   - 您會在清單中看到 **「SubFirst — YouTube 訂閱優先」**！
...
3. **驗證首頁置頂效果**：
   ...
   - 您會驚喜地發現，在原生影片列表上方，強行浮現了一個具有毛玻璃發光特效的 **「訂閱優先」專區**，展示您最愛主播的所有未讀新影片！
   - 點擊卡片右上角 `✔️` 按鈕，體驗卡片平滑淡出的 Dismiss 爽快感！
4. **驗證播放頁側邊欄效果**：
   - 隨意點開任何一部 YouTube 影片。
   - 在右側推薦影片欄的最上方，會置頂出現一個 **「🛡️ 訂閱優先」** 模組，引導您去觀看關注頻道的其他未讀新片，杜絕演算法無限推薦的干擾！
